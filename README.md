...
### day20
##### 总结：实现拖拽移动，要监听三个动作事件，onmousedown（鼠标点下--该圆），onmousemove（鼠标移动--window），onmouseup（鼠标松开--window）

### day21
##### 总结：之前一直不知道闭包到底在实际应用中有啥作用，通过这个例子，知道闭包可以将变量保存在内存流中，需要用到的时候再调用。函数执行后,变量的改变,会一直保存在内存流中,再次调用时是改变的值; (同一个变量接收的函数调用,不同变量接收同一个函数,还是会以初始值变量)。例子中bool变成true之后，就一直都是true了
